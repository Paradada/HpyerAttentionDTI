# -*- coding: utf-8 -*-
"""
de_novo_e3.py —— 论文第 3.4 节「Performance evaluation under de novo setting」
实验 E3（novel protein，仅蛋白质 unseen）。
...
作用：
    ...
    仅将「数据集划分逻辑」替换为 de novo E3 定义：
        随机抽取 20% 的独特蛋白质作为 unseen，其所有交互构成测试集；
        剩余 DTI 按 4:1 随机划分为训练集 + 验证集。
    最终执行单次实验（不做 5 折、不做多次重复）。

用法：
    python de_novo_e3.py --dataset DrugBank --seed 123 --data_ratio 0.2
"""
...
# ======================================================================================
# 一、数据读取与 de novo E3 划分（本文件唯一新增逻辑）
# ======================================================================================
...
def split_de_novo_protein(lines, ratio, seed):
    """E3 划分：随机 ratio 的独特蛋白质为 unseen，其所有交互作为测试集。

    Args:
        lines (list[str]): 原始数据行列表。
        ratio (float): unseen 蛋白质比例（论文为 0.2）。
    ...
    n_unseen = max(1, int(ratio * len(unique_proteins)))
    unseen_proteins = set(rng.choice(unique_proteins, size=n_unseen, replace=False).tolist())

    test_lines = [l for l in lines if get_protein_id(l) in unseen_proteins]
    remain_lines = [l for l in lines if get_protein_id(l) not in unseen_proteins]
...
def run_experiment(args):
    ...
    # 4) de novo E3 划分（唯一与原始不同的地方）
    remain_lines, test_lines = split_de_novo_protein(lines, ratio, seed)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="HyperAttentionDTI de novo E3 (novel protein) experiment")
