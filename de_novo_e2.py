# -*- coding: utf-8 -*-
"""
de_novo_e2.py —— 论文第 3.4 节「Performance evaluation under de novo setting」
实验 E2（novel drug，仅药物 unseen）。
...
作用：
    ...
    仅将「数据集划分逻辑」替换为 de novo E2 定义：
        随机抽取 20% 的独特药物作为 unseen，其所有交互构成测试集；
...
# ======================================================================================
# 一、数据读取与 de novo E2 划分（本文件唯一新增逻辑）
# ======================================================================================
...
def split_de_novo_drug(lines, ratio, seed):
    """E2 划分：随机 ratio 的独特药物为 unseen，其所有交互作为测试集。

    Args:
        lines (list[str]): 原始数据行列表。
        ratio (float): unseen 药物比例（论文为 0.2）。
    ...
    n_unseen = max(1, int(ratio * len(unique_drugs)))
    unseen_drugs = set(rng.choice(unique_drugs, size=n_unseen, replace=False).tolist())

    test_lines = [l for l in lines if get_drug_id(l) in unseen_drugs]
    remain_lines = [l for l in lines if get_drug_id(l) not in unseen_drugs]
...
def run_experiment(args):
    ...
    # 4) de novo E2 划分（唯一与原始不同的地方）
    remain_lines, test_lines = split_de_novo_drug(lines, ratio, seed)
