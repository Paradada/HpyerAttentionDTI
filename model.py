...
class AttentionDTI(nn.Module):
    def __init__(self,hp,
        ...
        self.Drug_CNNs = nn.Sequential(
            nn.Conv1d(in_channels=self.dim, out_channels=self.conv_channels[0], kernel_size=self.drug_kernel[0]),
            nn.ReLU(),
            nn.Conv1d(in_channels=self.conv_channels[0], out_channels=self.conv_channels[1], kernel_size=self.drug_kernel[1]),
            nn.ReLU(),
            nn.Conv1d(in_channels=self.conv_channels[1], out_channels=self.conv_channels[2], kernel_size=self.drug_kernel[2]),
            nn.ReLU(),
        )
        self.Drug_max_pool = nn.MaxPool1d(self.drug_MAX_LENGH-self.drug_kernel[0]-self.drug_kernel[1]-self.drug_kernel[2]+3)
        self.Protein_CNNs = nn.Sequential(
            nn.Conv1d(in_channels=self.dim, out_channels=self.conv_channels[0], kernel_size=self.protein_kernel[0]),
            nn.ReLU(),
            nn.Conv1d(in_channels=self.conv_channels[0], out_channels=self.conv_channels[1], kernel_size=self.protein_kernel[1]),
            nn.ReLU(),
            nn.Conv1d(in_channels=self.conv_channels[1], out_channels=self.conv_channels[2], kernel_size=self.protein_kernel[2]),
            nn.ReLU(),
        )
        ...
        self.attention_layer = nn.Linear(self.conv_channels[2], self.conv_channels[2])
        self.protein_attention_layer = nn.Linear(self.conv_channels[2], self.conv_channels[2])
        self.drug_attention_layer = nn.Linear(self.conv_channels[2], self.conv_channels[2])
        ...
        self.fc1 = nn.Linear(self.conv_channels[2] * 2, 1024)
